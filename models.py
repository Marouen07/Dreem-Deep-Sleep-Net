...
class Feature_extractor(nn.Module):
  def __init__(self, n_measures,fs, drop=.5,seq_l=30,features=False):
    ...
    self.temporal = nn.Sequential(
        nn.Conv1d(self.measures,128, fs//2+1,fs//10),nn.ReLU(inplace=True),nn.BatchNorm1d(128),nn.MaxPool1d(8,stride=8),nn.Dropout(drop),
    ...
    self.temp_len=int(int(((seq_l*fs-fs//2-1)/(fs//10))+1)/8) #first layer of Conv
    ...
    self.n_features=(self.freq_len+self.temp_len)*128
...
class Seq_learn(nn.Module):
  def __init__(self,hidden_size,Residual=True,n_layers=2,lookback=3, drop=.5):
    ...
    self.lstm=nn.LSTM(input_size=self.hidden_size,hidden_size=self.hidden_size,
                ...
                bidirectional =True)
    self.lin=nn.Sequential(nn.Dropout(drop),nn.Linear(3*self.hidden_size*self.lookback,5))
